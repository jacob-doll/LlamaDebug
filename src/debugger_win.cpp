...
bool ld_debug_open(char* target) {
    ...
    if (!create_interfaces()) return false;
    if (g_context.Symbols->SetSymbolPath(
        "cache*;srv*https://msdl.microsoft.com/download/symbols") != S_OK) {
        return false;
    }
    ...
    if (g_context.Client->CreateProcess(
        0, 
        target, 
        DEBUG_ONLY_THIS_PROCESS | CREATE_NEW_CONSOLE) != S_OK) {
        return false;
    }
    return true;
}
...
int ld_debug_wait() {
    ...
    if (FAILED(status)) return LD_STATUS_DEAD;
...
uint32_t ld_read_virtual(uintptr_t offset, uint8_t* buffer, size_t size) {
    ...
    if (g_context.DataSpaces->ReadVirtual(
        offset, buffer, (ULONG) size, &BytesRead) != S_OK) {
        return 0;
    }
    return BytesRead;
}
...
ld_breakpoint* ld_add_breakpoint(uintptr_t addr) {
    ...
    while ((status = g_context.Control->AddBreakpoint(
        DEBUG_BREAKPOINT_CODE, index, &bp)) == E_INVALIDARG) {
        ++index;
    }
    if (status != S_OK) {
        return nullptr;
